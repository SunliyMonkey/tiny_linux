...
修改配置，使用静态编译，如果不使用静态编译，程序运行期间需要加载共享库当中的函数，而在根文件系统运行时，显然没有共享库供其加载。
    
    Location:                                                                -> Busybox Settings                                        
       -> Build Options     
...
观察上面的错误，可以发现问题出在`inetd.c`中有未定义的引用，在网上搜索一下答案，关闭配置当中的`inet`选项即可忽略该问题
        ...
        Location:                                                                -> Networking Utilities 
            [ ] inetd  
            
这时再执行`make`，`make install`就能成功生成`_install`目录。
...
原来config当中已经提供了该编译优化选项，在`make menuconfig`当中搜索一下：`optimize`
     ...
     Location:                                                    
        (1) -> General setup                                                       Defined at init/Kconfig:1290     
...
接下来就是进一步精简该选项当中的配置，围绕着`网络驱动，TCP/IP通信支持`，能够在网络设置、驱动设置当中去掉绝大部分的选项。
 ...
 >PS：在打开的过程当中，会出现一些相关联的选项也被打开，同时allnoconfig配置当中的一些选择也可以进行删除，以下是必备的选项，其余相关联的选项，可以关闭。
